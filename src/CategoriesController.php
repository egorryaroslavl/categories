<?php
	...
	use Egorryaroslavl\Categories\Models\CategoryModel;
	...
	class CategoriesController extends Controller
	{
		...
		public function index()
		{
			...
			$data        = CategoryModel::orderBy( 'pos', 'ASC' )
				->paginate( config( 'admin.categories.paginate' ) );
			$data->table = 'categories';
			$breadcrumbs = '<div class="row wrapper border-bottom white-bg page-heading"><div class="col-lg-12"><h2>Категории</h2><ol class="breadcrumb"><li><a href="/admin">Главная</a></li><li class="active"><a href="/admin/categories">Категории</a></li></ol></div></div>';
		...
		public function create()
		{
			...
			$data->act   = 'categories-store';
			$data->table = 'categories';

			$breadcrumbs = '<div class="row wrapper border-bottom white-bg page-heading"><div class="col-lg-12"><h2>Категории</h2><ol class="breadcrumb"><li><a href="/admin">Главная</a></li><li class="active"><a href="/admin/categories">Категории</a></li><li><strong>Создание новой категории</strong></li></ol></div></div>';

			return view( 'categories::form', [ 'data' => $data, 'breadcrumbs' => $breadcrumbs ] );
		...
		public function store( Request $request )
		{
			...
			$categoryModel = CategoryModel::create( $input );
			$id            = $categoryModel->id;
			...
			if( !empty( $request->icon ) && !strpos( $request->icon, 'upload' ) ){
				...
				$fileName = config( 'admin.categories.icons_dir' ) . str_replace( $request->_token, $id, $baseName );
				/* абсоютный путь */
				$filePath = public_path( $fileName );
				/* также делаем для превью */
				$fileNameSmall = config( 'admin.categories.icons_dir' ) . str_replace( $request->_token, $id . '_small', $baseName );
				...
				Image::make( $request->icon )
					->save( $filePath )
					->widen( config( 'admin.categories.icon_width' ), function ( $constraint ){
						$constraint->upsize();
					} )
					->heighten( config( 'admin.categories.icon_height' ), function ( $constraint ){
						$constraint->upsize();
					} )->save( $filePathSmall );
		...
		public function edit( $id )
		{
			...
			$breadcrumbs = '<div class="row wrapper border-bottom white-bg page-heading"><div class="col-lg-12"><h2>Категории</h2><ol 
class="breadcrumb"><li><a href="/admin">Главная</a></li><li 
class="active"><a href="/admin/categories">Категории</a></li><li>Редактирование <strong>[
 <a href="/categories/' . $data->alias . '" style="color:blue" title="Смотреть на пользовательской части">' . $data->name . ' <img src="/_admin/img/extlink.png" alt="" 
 style="margin:0"></a> ]</strong></li></ol></div></div>';
		...
		public function update( Request $request )
		{
			...
			if( !empty( $request->icon ) && !strpos( $request->icon, 'upload' ) ){
				...
				$baseName = basename( $request->icon );
				/* в /tmp файл имеет в имени _token Меняем его не id категории и прибавляем к нему путь для сохранения иконок */
				$fileName = config( 'admin.categories.icons_dir' ) . str_replace( $request->_token, $request->id, $baseName );
				/* абсоютный путь */
				$filePath = public_path( $fileName );
				/* также делаем для превью */
				$fileNameSmall = config( 'admin.categories.icons_dir' ) . str_replace( $request->_token, $request->id . '_small', $baseName );
				/* абсоютный путь для превью */
				$filePathSmall = public_path( $fileNameSmall );
